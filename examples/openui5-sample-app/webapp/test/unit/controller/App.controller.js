  ...
  (
    ...
    QUnit.test("Check controller's initial state", (assert) => {
      ...
      assert.deepEqual(
        ...
        [],
        "Search filters have been instantiated empty"
      );
      assert.deepEqual(
        oAppController.aTabFilters,
        [],
        "Tab filters have been instantiated empty"
      );

      const oModel = oAppController.getView().getModel("view").getData();
      assert.deepEqual(oModel, {
    ...
    QUnit.test("Should add a todo element to the model", (assert) => {
      ...
      assert.strictEqual(
        oJSONModelStub.getObject("/todos").length,
        0,
        "There must be no todos defined."
      );

      // Act
      oJSONModelStub.setProperty("/todos", [
        { title: "Completed item", completed: true },
      ]);
      ...
      assert.strictEqual(
        oJSONModelStub.getObject("/todos").length,
        2,
        "There are couple items in ToDo list."
      );
    });
    ...
    QUnit.test("Should toggle the completed items in the model", (assert) => {
      ...
      assert.strictEqual(
        oJSONModelStub.getProperty("/itemsLeftCount"),
        1,
    ...
    QUnit.test(
      ...
      (assert) => {
        ...
        // initial assumption
        assert.strictEqual(
          oJSONModelStub.getObject("/todos"),
          undefined,
          "There are no items."
        );
        assert.strictEqual(
    ...
    QUnit.module("Test search", {
      beforeEach() {
        ...
        sinon.stub(Controller.prototype, "getView").returns(oViewStub);
        sinon.stub(Controller.prototype, "byId").returns(oListStub);
        sinon.stub(oListStub, "getBinding").returns({ filter() {} });
    ...
    QUnit.test("Empty search", (assert) => {
      ...
      assert.strictEqual(
        oAppController.getView().getModel().getProperty("/itemsRemovable"),
        true,
